...
export class Esfera2 {
    ...
    constructor(gl, radio, segmentosH, segmentosV) {
        ...
        for (let i = 0; i < cantidadDeVertices; i++) {
            const z = vertices[i * 3 + 2]; // altura del vértice
            const intensidad = (z / radio + 1) / 2; // 0..1

            // Exagera la diferencia: más contraste
            const r = 0.0;
            const g = 0.2 + 0.8 * intensidad; // entre 0.2 y 1.0
            const b = 0.2 * (1.0 - intensidad); // algo de azul oscuro en partes bajas
